...
export default function FolderPage() {
  ...
  return (
    <div
      className="bg-[#e5e5e5] dark:bg-[#1a1a1a] font-inter text-black dark:text-white min-h-screen relative flex selection:bg-black selection:text-white dark:selection:bg-white dark:selection:text-black transition-colors"
    >
      ...
      <aside className="hidden md:flex flex-col fixed left-0 top-0 bottom-0 p-4 gap-4 h-screen w-64 bg-[#f5f5f5] dark:bg-[#0a0a0a] border-r-2 border-black dark:border-white z-40 transition-colors">
        ...
        <nav className="flex-1 mt-3 flex flex-col gap-1.5 overflow-y-auto px-1">
          <Link className="flex items-center gap-2.5 text-gray-600 dark:text-gray-400 hover:text-black dark:hover:text-[#0a0a0a] hover:bg-black/5 dark:hover:bg-white hover:border-black dark:hover:border-white border-2 border-transparent px-3 py-2 transition-colors" to="/dashboard">
          ...
          {folders.map(f => {
            ...
            return (
               <Link 
                ...
                to={`/folder/${f._id}`}
              >
                <span className="material-symbols-outlined text-[18px]">{isActive ? 'folder_open' : 'folder'}</span>
      ...
      <main className="md:ml-64 min-h-screen relative flex flex-col w-full z-10">
        ...
        <section className="pt-24 px-6 pb-12 flex-1 max-w-6xl mx-auto w-full relative z-10">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
            <div>
              <nav className="flex items-center gap-2 text-[9px] font-black text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-2">
                <Link to="/dashboard" className="hover:text-black dark:hover:text-white underline decoration-2 underline-offset-2">FDIR</Link>
                <span className="material-symbols-outlined text-xs font-bold">chevron_right</span>
                <span className="text-black dark:text-white">{activeFolder?.name || 'Folder'}</span>
              </nav>
              ...
              <p className="text-gray-600 dark:text-gray-300 font-bold max-w-xl uppercase tracking-widest text-[10px] border-l-[2px] border-black dark:border-white pl-3">
                Structured repository designated for encapsulated nodes and active snippet retrieval.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex bg-[#f5f5f5] dark:bg-[#0a0a0a] border-2 border-black dark:border-white shadow-[3px_3px_0_0_rgba(0,0,0,1)] dark:shadow-[3px_3px_0_0_rgba(255,255,255,1)] overflow-hidden">
                <button className="px-2 py-1.5 bg-black dark:bg-white text-white dark:text-black border-r-2 border-black dark:border-white">
                  <span className="material-symbols-outlined font-bold text-lg">grid_view</span>
                </button>
                <button className="px-2 py-1.5 text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white hover:bg-black/10 transition-colors">
                  <span className="material-symbols-outlined font-bold text-lg">format_list_bulleted</span>
                </button>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 items-start">
            {loading ? (
              <div className="col-span-3 py-16"><Loader size="lg" text="Syncing blocks..." /></div>
            ) : items.length === 0 ? (
               <div className="col-span-3 text-center border-2 border-dashed border-gray-400 dark:border-gray-600 p-10 bg-white/50 dark:bg-black/50">
                   <p className="text-sm font-black uppercase tracking-widest text-gray-500 dark:text-gray-500 mb-4">Sector clear. Ready for commit.</p>
                   <button onClick={handleAddItem} className="px-5 py-2.5 bg-[#e5e5e5] dark:bg-[#1a1a1a] border-2 border-black dark:border-white text-black dark:text-white font-black text-xs uppercase tracking-widest hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-all">INITIALIZE</button>
               </div>
            ) : items.map((item, idx) => (
              <ItemCard
                key={item._id}
      ...
      <nav className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] bg-[#f5f5f5] dark:bg-[#0a0a0a] border-2 border-black dark:border-white flex justify-around items-center py-3 shadow-[6px_6px_0_0_rgba(0,0,0,1)] dark:shadow-[6px_6px_0_0_rgba(255,255,255,1)] z-50 transition-colors">
        <Link className="text-gray-500 hover:text-black dark:hover:text-white flex flex-col items-center gap-0.5 transition-colors" to="/dashboard">
        ...
        <div className="relative -top-6 ml-auto mr-auto">
          <button onClick={handleAddItem} className="w-12 h-12 bg-black dark:bg-white text-white dark:text-black border-2 border-black dark:border-white rounded-none shadow-[4px_4px_0_0_rgba(0,0,0,1)] dark:shadow-[4px_4px_0_0_rgba(255,255,255,1)] flex items-center justify-center transform transition-transform active:translate-y-0.5 active:shadow-none">
